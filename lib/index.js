...
import { BROWSER_AUTO_OPEN_DISABLED_LINE, OAUTH_ORIGIN_LINE_PREFIX, ZENMUX_BROWSER_STATUS_PATH, } from './shared.js';
/** Cordis plugin name. */
...
function parseMetadata(record, expectedOrigin) {
...
class ZenMuxOAuthController {
    ...
    async refreshForStatus() {
        const tokenSet = this.tokenSet;
        if (tokenSet === undefined || tokenSet.expiresAt - Date.now() > this.config.refreshSkewMs)
            return;
        try {
            ...
            this.ctx.logger.warn('zenmux: token refresh on status failed: %s', error instanceof Error ? error.message : String(error));
